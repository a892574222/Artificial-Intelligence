...
public class Cost_SMO {
	...
	//C1表+的惩罚，C2表-的惩罚
	public Cost_SMO(double[][]data,Integer[] label,Double C1,Double C2,Double toler,String kernel) {
		this.data = data;
		this.label = label;
		this.C1 = C1;
		this.C2 = C2;
	...
	private double clipalpha(int a1, int a2) {
		...
		if(label[a1]==1) {
			if(label[a2]==1) {
				L = Math.max(0, alphas[a2]+alphas[a1]-C1);
				H = Math.min(C1, alphas[a2]+alphas[a1]);
			}else {
				L = Math.max(0, alphas[a2]-alphas[a1]);
				H = Math.min(C2, C1+alphas[a2]-alphas[a1]);
			}
		}else {
			if(label[a2]==1) {
				L = Math.max(0, alphas[a2]-alphas[a1]);
				H = Math.min(C1, C2+alphas[a2]-alphas[a1]);
			}else {
				L = Math.max(0, alphas[a2]+alphas[a1]-C2);
				H = Math.min(C2, C1+alphas[a2]);
			}
		}
