...
public class NBSVM {
	...
	private OVA[][] finally_svm;
	
	public NBSVM(double[][][][] training_data_x, Integer[][][] training_data_y, double KKTviolationsLevel, double[][][][][] test_data_x,
			Integer[][][] test_data_y) {
		this.test_data_x = test_data_x;
		this.test_data_y = test_data_y;
		finally_svm = new OVA[test_data_x.length][test_data_x[0].length];
		this.select_index(training_data_x, training_data_y, KKTviolationsLevel);
		
	}
	
	private void select_index(double[][][][] training_data_x,Integer[][][] training_data_y, double KKTviolationsLevel) {
		double[] Cost = {1,10,100};
		...
		for(int times=0;times<training_data_x.length;times++) {
			for(int k=0;k<training_data_x[times].length;k++) {
				gridsearch = new GridSearch("NBSVM",KKTviolationsLevel,training_data_x[times][k],training_data_y[times][k],Cost,Sigma,Weight,5);
				finally_svm[times][k] = new OVA(training_data_x[times][k],training_data_y[times][k],KKTviolationsLevel,gridsearch.get_best_cost(),gridsearch.get_best_kernel(),gridsearch.get_best_sigma(),gridsearch.get_best_weight());
